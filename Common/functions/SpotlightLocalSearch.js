...
function groupedResults(matches, query, labels, capacities, retainedId) {
    ...
    // ЛОКАЛЬНАЯ ПРАВКА: было ["files", "web"] — строка поиска по файлам
    // убрана, остался только веб-поиск.
    ["web"].forEach((kind, index) => results.push({id: "extension:" + kind, provider: "extension",

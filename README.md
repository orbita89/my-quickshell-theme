...
Автозапуск — служба `my-shell.service` (файл лежит в
`~/.config/systemd/user/`, вне этого репозитория).

В ней есть зачистка хвостов: до запуска и после остановки выполняется

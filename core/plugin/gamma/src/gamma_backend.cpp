...
class Control : public QObject, public QtWayland::zwlr_gamma_control_v1 {
  public:
    ...
    {
        // ЛОКАЛЬНАЯ ПРАВКА: было 65536. Intel Tiger Lake отдаёт
        // сегментированную таблицу гаммы на 262145 значений, и проверка
        // молча браковала её: плагин сам слал failed и уничтожал управление,
        // поэтому ночной режим не работал, а в логе композитора было пусто.
        if (value < 2 || value > 1048576) {
